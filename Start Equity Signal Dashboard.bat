...
REM When the server is already running: double-click "Open Equity Signal Dashboard.url"
REM (opens the pipeline page in your browser — it does not start the server).
setlocal EnableExtensions
title Equity Signal — Advisor app (local)
...
if not defined WEALTH_LEADS_APP_SECRET (
    set "WEALTH_LEADS_APP_SECRET=equity-signal-local-dev-only-change-me"
    echo.
...
echo Close the existing Equity Signal console when you want to stop the server.
exit /b 0
...
if not "%_ERR%"=="0" (
    ...
    echo.
    echo If port 8765 is already in use, close the other Equity Signal window or run:
